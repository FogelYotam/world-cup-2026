...
- `config.py` — settings; loads `.env`. Key knobs: `REPORT_UPCOMING_COUNT` (=5 matches in the report), `POSITION_PICKS_PER_POS`, `TRANSFER_CANDIDATES_PER_POS` (=2), `MARKET_BLEND_WEIGHT`, `ODDS_REVEAL_HOURS`.
  ...
  - `ODDS_SOURCES` — the **top-5 sharpest** bookmaker names for the consensus odds query (Pinnacle, Betfair Exchange, Bet365, Circa, Oddschecker), handed to Gemini as grounded-search hints; the consensus 1X2 blends into the prediction via `MARKET_BLEND_WEIGHT`.
