...
- `scraper.py` — data collection via **Gemini grounded search**. `GeminiClient` (`ask_json`, `ask_json_image` for Vision). Fetches matches/teams/match-context/fantasy pool. `ingest_results()` learns from finished scores (EWMA into team goals → improves **predictions**). `ingest_player_results()` learns from real player performances (goals/assists/minutes/clean-sheet → computes actual FIFA points, EWMA into each player's `recent_points` → improves **fantasy** picks; auto-adds standout players missing from the pool). **Both ingest functions now run inside `collect()` (both branches), after the scrape**, so the daily report keeps learning — they are not gated to the bot's ~5h refresh. `_enrich_fantasy_data()` pulls prices/form/xG from `FANTASY_SOURCES`.
- `fantasy.py` — `expected_points` now blends each player's `recent_points` (actual recent FIFA-points signal from `ingest_player_results`) at 0.6 model / 0.4 actual·start-prob, so picks/captain/differentials reflect who actually performed.
