...
- `backtest.py` — backtesting harness. Replays `db['results']` through `predictor.predict_match` and scores under KICKOFF (`match_points`) vs two naive baselines (always 1-0 home, always 1-1). `run_backtest(db, scoring)` → metrics (ppg/exact/direction) per variant; `format_report`. Use for A/B before deploying a model change. In-sample (model learned from these results) — compare *relatively* between variants. CLI: `python backtest.py`.
