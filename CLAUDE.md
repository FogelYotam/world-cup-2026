...
- `fantasy.py` — `expected_points` blends **xG/xA** into the goals/assists per-match rates (50/50 — xG is a more stable predictor than raw goals, neutralising finishing luck), then blends each player's `recent_points` (actual recent FIFA-points signal from `ingest_player_results`) at 0.6 model / 0.4 actual·start-prob, so picks/captain/differentials reflect both underlying threat and who actually performed.
