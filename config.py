...
MAX_XG = 3.5                   # תקרת צפי שערים לקבוצה. הורד מ-4.5 → 3.5: ב-4.5 נוצרה
                               # *ערימת ניחושי "4"* (17% מהקבוצות מול 11% במציאות) והממוצע
                               # היה 1.73 מול 1.52 אמיתי. ב-3.5: ממוצע 1.57≈מציאות, ניחושי
                               # 4+ צונחים ל-1%, ו-ppg אף *עולה* (1.20 מול 1.16). בלי הגזמה.
...
# תיקון Dixon-Coles לתלות בתוצאות נמוכות (0=פואסון עצמאי). ρ שלילי מגביר תיקו
# ומקטין 1-0/0-1. נבחר −0.05 אמפירית מול 54 התוצאות: משפר כיול (Brier 0.3944→0.3926,
# פגיעת-פייבוריט 70%→72%) **בלי לפגוע ב-ppg** (ρ שלילי יותר משפר Brier אך מגביר תיקו
# ופוגע בניקוד, כי המודל כבר נוטה לתיקו).
...
# בסיס שלב-הבתים בטבלת "אתה מול המערכת" — מעוגן למספרים הרשמיים מהאפליקציה (המעקב
# האוטומטי כיסה רק 60/72 משחקי בתים). משתמש 62 (רשמי); מודל 58 (אומדן הוגן).
# משחקי נוקאאוט (R32+) נספרים ישירות מעל הבסיס עם מכפילי השלב (×2/×3).
PREDICTION_GROUP_BASELINE = {"user": 62, "model": 58}
...
# משקל ה-scouting bonus בדירוג הדיפרנציאלים. גבוה → דיפרנציאלים *טובים* מתחת ל-5%
# (תוחלת ניקוד גבוהה + סיכוי העפלה דרך קלות-המשחק) מודגשים בראש הרשימה בדוח.
DIFFERENTIAL_SCOUTING_WEIGHT = 3.0
# אם הקפטן הנוכחי עשה כך וכך נקודות במחזור האחרון ומעלה — לא ממליצים להחליפו ("בּוֹעֵר").
CAPTAIN_KEEP_THRESHOLD = 9.0
...
# משקל השוק מול המודל (0=מודל בלבד, 1=שוק בלבד). הועלה ל-0.8 — הבוקמייקרים החדים
# (Pinnacle/Betfair) כבר מגלמים פורם+מוניטין+סגל+פציעות, בעוד המודל הפנימי "עיוור
# למוניטין" ובטוח-יתר על פורם של 3 משחקים (למשל CIV>Norway, Mexico>England). כשיש
# הימורים — הם מובילים; המודל נשאר רק כשובר-שוויון ולמשחקים בלי הימורים.
MARKET_BLEND_WEIGHT = 0.8
...
_TUNING_BOUNDS = {"MAX_XG": (3.0, 3.5), "HOME_ADVANTAGE": (0.0, 0.4),
                  "MARKET_BLEND_WEIGHT": (0.7, 0.9)}
